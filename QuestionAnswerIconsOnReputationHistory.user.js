...
(function () {
    ...
    const main = () => {
        $(`${config.questionLinkSelector}:not(.${config.hasIconClass})`)
            .not(`.${config.hasIconClass}`)
            .addClass(config.hasIconClass)
            .before(config.questionIcon.clone());


        $(`${config.answerLinkSelector}:not(.${config.hasIconClass})`)
            .addClass(config.hasIconClass)
            .before(config.answerIcon.clone());
    };
    ...
    StackExchange.ready(() => {
        ...
        // Watch for data-load expandable to add jsloaded class to element (done on load completion)
